...
public class FunctionCallingChatConfiguration {
    ...
    @Value("classpath:/prompts/ppe.st")
    ...
    ChatClient functionCallingChatClient(ChatClient.Builder chatClientBuilder, ChatMemory functionCallingChatMemory) {
        return chatClientBuilder
                .defaultAdvisors(
                        ...
                        //new ErrorHandlingAdvisor() // Add error handling advisor
                )
                .defaultSystem(functionCallingPromptResource)
                //.defaultOptions(ChatOptions.builder().temperature(0.1).maxTokens(150)
                //      .topP(0.9).frequencyPenalty(0.5).presencePenalty(0.6)
                //        .build())
                .build();
    }
